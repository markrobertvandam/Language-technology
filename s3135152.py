        ...
        if place > 0 and token.text != "is" and token.text != "are" and token.text != "was":
            ...
            if token.dep_ == "amod":
                try:
                    ...
                    elif (parse[i + 1].dep_ == "amod" and parse[i + 2].dep_ == "nsubj") or (
                            parse[i + 1].dep_ == "compound" and (
                            parse[i + 2].dep_ == "dobj" or parse[i + 2].dep_ == "nsubj")):
        ...
        for i in range(len(entities)):
            for p in range(min(10, len(properties))):
                ...
                data = requests.get('https://query.wikidata.org/sparql',
                                    params={'query': select_query, 'format': 'json'}).json()
...
def main():
    ...
    example_questions = [
        "Who is the CEO of Sony Music?", "Who are the members of the Beatles?",
        "Who was the composer of the moonlight sonata?", "What are Queen its genres?",
        "What was the place of death of David Bowie?", "State Sun Studio its location.",
        "State Bono his birthname?", "State Elton John his sexuality.",
        "Tell me te height of Roger Daltrey.", "Tell me the record labels of linkin park"
    ]
